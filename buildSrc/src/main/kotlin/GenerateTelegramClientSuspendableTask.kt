...
abstract class GenerateTelegramClientSuspendableTask : AbstractGeneratorTask() {
    ...
    @Internal override val targetFileName = "TelegramClientSuspendable"
    ...
    fun execute() {
        ...
        val classBuilder = TypeSpec.interfaceBuilder(targetFileName)
            .addSuperinterface(TelegramClient::class)

        val methods = classDecl.methods.filter {
            !it.type.asString().startsWith("CompletableFuture")
        }

        methods.forEach { method ->
            val returnType = method.type.toKotlinTypeName(method, symbolResolver)

            val funSpec = FunSpec.builder(method.nameAsString + "Suspend")
                ...
                .apply {
                    ...
                    addStatement("return %L(%L).await()", "${method.nameAsString}Async", paramNames)
                }
                .build()
